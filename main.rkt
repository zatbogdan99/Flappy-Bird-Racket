...
(struct bird-struct (x y v-y) #:transparent);
(struct pipe-struct (x gap-y) #:transparent)
(struct variables-struct (gravity momentum scroll-speed) #:transparent)
(struct abilities-struct (visible active) #:transparent)

(struct stareJoc (bird-struct pipes variable-struct score)
  #:transparent)
...
(define (get-initial-state)
  (stareJoc (bird-struct bird-x bird-initial-y 0)
            (list (pipe-struct scene-width (+ added-number (random random-threshold))))
            (variables-struct initial-gravity initial-momentum initial-scroll-speed)
            0))
...
(define (next-state-bird bird gravity)
  (struct-copy bird-struct bird [y (+ (bird-struct-y bird) (bird-struct-v-y bird))]
               [v-y (+ (bird-struct-v-y bird) gravity)]) )
...
(define (next-state-bird-onspace bird momentum)
  (struct-copy bird-struct bird [v-y (- 0 momentum)]))
...
(cond
  [(key=? pressed-key " ") (struct-copy stareJoc current-state [bird-struct (next-state-bird-onspace (stareJoc-bird-struct current-state) initial-momentum)])]
  [else current-state]))
...
(define (move-pipes pipes scroll-speed)
  (map (λ (pip) (struct-copy pipe-struct pip [x (- (pipe-struct-x pip) scroll-speed)])) pipes))
...
; cu pasărea.
;(pipe-struct-gap-y (last pipes))

(define (add-more-pipes pipes)
  (if (>= (length pipes) no-pipes)
      pipes
      (append pipes
            (list (pipe-struct (+ (+ pipe-width pipe-gap) (pipe-struct-x (last pipes))) 
...
(define (invalid-state? state)
  (if (or (check-ground-collision (stareJoc-bird-struct state))
          (check-pipe-collisions (stareJoc-bird-struct state) (stareJoc-pipes state)))
      #t
      #f
      ))
...
;(- (pipe-struct-gap-y (car pipes)) pipe-height)

(define (check-pipe-collisions bird pipes)
  (if (= (length pipes) 0)
       #f
       (if (or (check-collision-rectangles (make-posn (bird-struct-x bird) (bird-struct-y bird))
                                  (make-posn (+ (bird-struct-x bird) bird-width) (+ (bird-struct-y bird) bird-height))
                                  (make-posn (get-pipe-x (car pipes)) 0)
                                  (make-posn (+ (get-pipe-x (car pipes)) pipe-width) (pipe-struct-gap-y (car pipes)))
                                  )
      (check-collision-rectangles (make-posn (bird-struct-x bird) (bird-struct-y bird))
                                  (make-posn (+ (bird-struct-x bird) bird-width) (+ (bird-struct-y bird) bird-height))
                                  (make-posn (get-pipe-x (car pipes)) (+ (pipe-struct-gap-y (car pipes)) pipe-self-gap))
                                  (make-posn (+ (get-pipe-x (car pipes)) pipe-width) scene-height)
                                  )
      )
      #t
...
(define (next-state state)
  (struct-copy stareJoc state [bird-struct (next-state-bird (stareJoc-bird-struct state) initial-gravity)]
               [pipes (next-state-pipes (stareJoc-pipes state) initial-scroll-speed)]
               [score (+ (stareJoc-score state) 0.1)]
               ))
;state)
; draw-frame
...
(define (draw-frame state)
  (place-image bird-image (+ (bird-struct-x (stareJoc-bird-struct state))
                             (quotient bird-width 2))
               (+ (bird-struct-y (stareJoc-bird-struct state)) (quotient bird-height 2))
               (place-image ground-image (quotient scene-width 2) (- scene-height (quotient ground-height 2))
                            (place-image (score-to-image (stareJoc-score state)) (quotient text-x 2) (quotient text-y 2)
                                         (place-pipes (stareJoc-pipes state)
                                         (place-image initial-scene (quotient scene-width 2) (quotient scene-height 2) (empty-scene scene-width scene-height))))))
  )
...
(define (place-pipes pipes scene)
  (if (= (length pipes) 1)
      ;pune ultima imagine
      (place-image gap-image (+ (pipe-struct-x (car pipes)) (quotient pipe-width 2))
                   (+ (pipe-struct-gap-y (car pipes)) (quotient pipe-self-gap 2))
                   (place-image pipe-image (+ (pipe-struct-x (car pipes)) (quotient pipe-width 2)) (quotient scene-height 2)
                                scene
                                ))
      ;le pune recursiv
      (place-image gap-image (+ (pipe-struct-x (car pipes)) (quotient pipe-width 2))
